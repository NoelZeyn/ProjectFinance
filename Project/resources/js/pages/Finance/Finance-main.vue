        ...
        <div class="bg-white p-6 rounded-lg shadow-xl w-96">
            ...
            <div class="flex flex-col gap-3">
              <label class="inline-flex items-center cursor-pointer">
                <input type="radio" v-model="exportType" value="monthly"
                  class="form-radio text-[#074a5d] focus:ring-[#074a5d]">
            ...
            <div v-if="exportType === 'monthly'" class="bg-gray-50 p-3 rounded border border-gray-200 mb-4">
              <div class="mb-3">
                ...
                <select v-model="exportMonth"
                  class="w-full border-gray-300 border p-2 rounded text-sm focus:border-[#074a5d] outline-none">
              ...
              <div>
                ...
                <input type="number" v-model="exportYear"
                  class="w-full border-gray-300 border p-2 rounded text-sm focus:border-[#074a5d] outline-none"
                  placeholder="Tahun">
              </div>
            </div>
          ...
          <div class="flex justify-end gap-3 mt-6">
            <button @click="showExportModal = false"
              class="px-4 py-2 bg-gray-200 text-gray-700 rounded hover:bg-gray-300 text-sm font-medium transition">
              Batal
            </button>
            <button @click="processExport"
              class="px-4 py-2 bg-[#074a5d] text-white rounded hover:bg-[#053341] text-sm font-medium shadow transition flex items-center gap-2">
...
export default {
  ...
  computed: {
    isAdmin() {
      return this.tingkatanOtoritas === 'admin' || this.tingkatanOtoritas === 'superadmin';
    },
    currentSourceList() {
      return this.isAdmin ? this.financeList : this.userFinanceList;
    },

    // 2. FILTER & SORT (Search Text & Date)
    filteredList() {
      let data = this.currentSourceList || [];

      return data.filter(item => {
        // A. Filter Search Text
        ...
        // B. Filter Date Picker
        // Cek apakah string tanggal item diawali dengan string date picker (YYYY-MM-DD)
        const dateMatch = !this.searchDate ||
          (item.date && item.date.toString().startsWith(this.searchDate));
    ...
    groupedFinanceByDate() {
      const listToGroup = this.paginatedList;
      const grouped = {};
